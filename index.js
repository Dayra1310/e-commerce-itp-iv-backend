...
import fs from 'fs';
import registrarModuloClientes from "./src/modulos-clientes/index-clientes.js";
import {
    loginUsuario,
...
app.get("/reportes/ventas", verificarToken, esUsuarioInterno, async (req, res) => {
    try {
        ...
        const resultados = await obtenerReporteVentas({ desde, hasta })
        if (!resultados.ok) {
            return res.status(500).json({ ok: false, message: resultados.message })
        }
...
app.get("/reportes/pedidos", verificarToken, esUsuarioInterno, async (req, res) => {
    try {
        const { desde, hasta } = req.query;
        const resultados = await obtenerReportePedidos({ desde, hasta })
        if (!resultados.ok) {
            return res.status(500).json({ ok: false, message: resultados.message })
        }
...
// ========== RUTAS INTEGRADAS: CLIENTES Y CATÁLOGO ==========
registrarModuloClientes(app);
